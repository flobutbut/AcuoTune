...
export function calculateFrequencyResponse(config: Config, electronicParams: ElectronicParams) {
  console.log('Calculating frequency response with:', { config, electronicParams })

  // Extraction de la pente du filtre (conversion de "XX dB/octave" en nombre)
  const penteString = electronicParams.penteFiltre
  const pente = parseInt(penteString)
  
  if (isNaN(pente)) {
    console.error('Invalid filter slope:', electronicParams.penteFiltre)
    return {
      voiceResponses: [],
      globalResponse: new Array(401).fill(0)
    }
  }
