  ...
  if (voiceIndex === 0) { // Grave
    ...
    switch(config.typeCharge) {
      case 'bass-reflex':
        ...
        if (frequency < fb) {
          const bassExtension = Math.min(12, 12 * Math.log10(frequency/fb))
          const qEffect = 3 * (Q - 0.5) * Math.exp(-Math.pow(Math.log(w), 2))
          response += bassExtension + qEffect
        } else {
          const qResonance = 2 * (Q - 0.5) * Math.exp(-Math.pow(Math.log(w), 2))
          response += qResonance
        }
        break;

      case 'double-bass-reflex':
        // Deux fréquences d'accord pour le double bass-reflex
        const fb1 = freqAccord > 0 ? freqAccord : (config.typeEnceinte === 'bibliothèque' ? 40 : 30)
        const fb2 = fb1 * 1.4 // Typiquement 40% plus haute
        
        const w1 = frequency / fb1
        const w2 = frequency / fb2
        
        if (frequency < fb1) {
          // Extension plus profonde dans les basses
          const bassExtension = Math.min(15, 15 * Math.log10(frequency/fb1))
          const qEffect = 2 * (Q - 0.5) * Math.exp(-Math.pow(Math.log(w1), 2))
          response += bassExtension + qEffect
        } else if (frequency < fb2) {
          // Zone entre les deux accords
          const midBassResponse = 3 * Math.exp(-Math.pow(Math.log(w1), 2))
          const qEffect = 2 * (Q - 0.5) * (Math.exp(-Math.pow(Math.log(w1), 2)) + Math.exp(-Math.pow(Math.log(w2), 2)))
          response += midBassResponse + qEffect
        } else {
          // Au-dessus de la seconde fréquence d'accord
          const qResonance = 1.5 * (Q - 0.5) * Math.exp(-Math.pow(Math.log(w2), 2))
          response += qResonance
        }
        break;
      ...
      case 'clos':
        ...
        if (frequency < fs) {
          ...
          const qEffect = 2 * (Q - 0.5) * Math.exp(-Math.pow(Math.log(w), 2))
          response += qEffect
        }
        break;
    }
