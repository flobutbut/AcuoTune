...
export function calculateVoiceResponse(frequency: number, voiceIndex: number, config: Config, electronicParams: ElectronicParams): number {
  console.log('Type de charge:', config.typeCharge) // Debug
  ...
  if (voiceIndex === 0) { // Grave
    ...
    // Effet de charge avec distinction claire entre bass-reflex et clos
    if (config.typeCharge === 'bass-reflex') {
      console.log('Applying bass-reflex response') // Debug
      const fb = freqAccord > 0 ? freqAccord : (config.typeEnceinte === 'bibliothèque' ? 45 : 35)
      const w = frequency / fb
      
      if (frequency < fb) {
        // Sous la fréquence d'accord : remontée caractéristique du bass-reflex
        response += 12 * Math.log10(frequency/fb)
        // Effet du Q sur la résonance
        response += 6 * Math.log10(1 + Math.pow(Q/w, 2))
      } else {
      ...
      console.log('Applying closed-box response') // Debug
      const fs = config.typeEnceinte === 'bibliothèque' ? 55 : 40
      const w = frequency / fs
      
      // Réponse caractéristique de la charge close
      if (frequency < fs) {
        // Pente -12dB/octave sous Fs
        response -= 12 * Math.log10(fs/frequency)
        // Effet du Q sur l'amortissement
        response += 3 * Math.log10(1 + Math.pow(Q*w, 2))
      }
    }
  ...
  console.log(`Response at ${frequency}Hz: ${response}dB`) // Debug
