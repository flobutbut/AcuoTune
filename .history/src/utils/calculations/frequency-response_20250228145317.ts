...
export function calculateVoiceResponse(frequency: number, voiceIndex: number, config: Config, electronicParams: ElectronicParams): number {
  ...
  const Q = parseFloat(electronicParams.facteurQ) || 0.707 // Facteur de qualité
  const freqAccord = parseFloat(electronicParams.frequenceAccord) || 0 // Fréquence d'accord
  ...
  if (voiceIndex === 0) { // Grave
    ...
    // Effet bass-reflex ou clos avec fréquence d'accord et Q
    if (config.typeCharge === 'bass-reflex' && freqAccord > 0) {
      // Réponse bass-reflex avec Q et fréquence d'accord
      const fb = freqAccord
      const w = frequency / fb
      
      // Modèle de Thiele-Small simplifié
      if (frequency < fb) {
        // Sous la fréquence d'accord
        const bassExtension = 12 * Math.log10(frequency/fb)
        const qEffect = 20 * Math.log10(Math.sqrt(1 + Math.pow(Q/w, 2)))
        response += bassExtension + qEffect
      } else {
        // Résonance autour de la fréquence d'accord
        const qResonance = Q * Math.exp(-Math.pow(Math.log(w), 2))
        response += qResonance
      }
    } else { // Charge close
      ...
      // Réponse charge close avec Q
      if (frequency < fs) {
        const rolloff = -12 * Math.log10(fs/frequency)
        const qEffect = 20 * Math.log10(Math.sqrt(1 + Math.pow(Q*w, 2)))
        response += rolloff + qEffect
      }
    }
