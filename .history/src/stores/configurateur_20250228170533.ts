...
export const useConfigurateurStore = defineStore('configurateur', {
  ...
  actions: {
    updateConfig(update: Partial<Config>) {
      ...
      // Si on change juste showAdvanced, on ne touche pas aux autres paramètres
      if (Object.keys(update).length === 1 && 'showAdvanced' in update) {
        this.config.showAdvanced = update.showAdvanced
        console.log('Store - After showAdvanced update:', { 
          showAdvanced: this.config.showAdvanced,
          typeCharge: this.config.typeCharge,
          facteurQualite: this.config.facteurQualite
        })
        return
      }
